...
def convert(hour, minute):
  if 0 < hour <= 12 and 0 < minute < 60:
    return formatReturn(hour, minute, 'A')
  elif 12 < hour < 24 and 0 < minute < 60:
    return formatReturn(hour - 12, minute, 'P')
  
  return print('Não foi possível fazer a conversão')

def formatReturn(hour, minute, schedule):
  if(schedule == 'P'):
    return print(f'{hour}:{minute}PM')
  
  return print(f'{hour}:{minute}AM')
